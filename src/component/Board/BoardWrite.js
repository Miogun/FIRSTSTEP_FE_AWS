...
const BoardWrite = () => {
    ...
    useEffect(() => {
        axios.get(`${REACT_APP_LAMBDA_API_URL}/checkid/${sessionStorage.getItem('token')}`)
        .then(responce => {
    ...
    const postData = () => {
        if (title === '') {
            ...
            axios.post(`${REACT_APP_LAMBDA_API_URL}/boardWrite`, {title : title, location : location, content : content, userId : userId }, { headers: { 'Content-Type': 'application/json' } })
            .then(response => {
