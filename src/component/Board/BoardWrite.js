...
const BoardWrite = () => {
    ...
    useEffect(() => {
        axios.get(`${process.env.REACT_APP_SERVER_URL}/checkid/${sessionStorage.getItem('token')}`)
        .then(responce => {
    ...
    const postData = () => {
        if (title === '') {
            ...
            axios.post(`${process.env.REACT_APP_SERVER_URL}/boardWrite`, {title : title, location : location, content : content, userId : userId }, { headers: { 'Content-Type': 'application/json' } })
            .then(response => {
